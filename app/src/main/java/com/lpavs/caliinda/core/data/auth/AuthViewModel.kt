...
class AuthViewModel @Inject constructor(
     ...
     * Мы используем stateIn, чтобы превратить StateFlow из синглтона AuthManager
     * в StateFlow, который живет в пределах viewModelScope. Это гарантирует,
     * что поток будет активен, пока на него есть подписчики (UI), и
     * остановится, когда ViewModel будет уничтожена, предотвращая утечки.
     *
     * SharingStarted.WhileSubscribed(5000) - стандартная практика: поток остается активным
     * еще 5 секунд после отписки последнего наблюдателя. Это помогает пережить
     * быстрые смены конфигурации (например, поворот экрана) без перезапуска потока.
     */
    ...
    /**
     * Предоставляет одноразовые события (например, успешный выход из аккаунта) для UI.
     * UI может подписаться на них для выполнения навигации или показа Snackbar.
     */
    val authEvents: SharedFlow<AuthEvent> = authManager.authEvents

    /**
     * Инициирует процесс входа в систему.
     * UI вызывает этот метод, передавая Activity, необходимую для запуска UI от Google.
     */
