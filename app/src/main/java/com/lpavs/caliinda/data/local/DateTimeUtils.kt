...
object DateTimeUtils {
  ...
  fun formatDateTimeToIsoWithOffset(
      ...
      time: LocalTime?,
      isAllDay:
          Boolean, // Этот параметр здесь, возможно, не так нужен, если isAllDay обрабатывается выше
      zoneIdString: String
  ): String? {
    ...
    val effectiveTime =
        if (isAllDay) LocalTime.MIDNIGHT else time!! // Если не all-day, time не должен быть null
